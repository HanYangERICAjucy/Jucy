...
function searchID($id)
{
    ...
    $resultset = mysql_query("select id, name, user_level, vote from user_record where id = '$id'");
    if (mysql_num_rows($resultset) < 1)
    {
        mysql_close($con);
        return array(array("empty"));
...
function searchNAME($name)
{
    ...
    $resultset = mysql_query("select id, name, user_level, vote from user_record where name = '$name'");
    if (mysql_num_rows($resultset) < 1)
    {
        mysql_close($con);
        return array(array("empty"));
...
function searchLEVEL($level)
{
    ...
    $resultset = mysql_query("select id, name, user_level, vote from user_record where user_level = '$level'");
    if (mysql_num_rows($resultset) < 1)
    {
        mysql_close($con);
        return array(array("empty"));
...
function searchDIDNTVOTE()
{
    ...
    $resultset = mysql_query("select id, name, user_level, vote from user_record where vote = 1");
    if (mysql_num_rows($resultset) < 1)
    {
        mysql_close($con);
        return array(array("empty"));
